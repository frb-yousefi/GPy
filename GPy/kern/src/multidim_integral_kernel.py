...
class Mix_Integral(Kern):
    ...
    def update_gradients_full(self, dL_dK, X, X2=None): 
        if X2 is None:  #we're finding dK_xx/dTheta
            lengthscale_gradients, variances_gradient = frb_update_gradients_full_X2_none(dL_dK, X, X2, np.array(self.lengthscale), np.array(self.variance))
...
@jit(nopython=True)
def frb_update_gradients_full(dL_dK, X, X2=None, lengthscale, variance): 
    if X2 is None:  #we're finding dK_xx/dTheta
        ...
        return lengthscale_gradients, variance_gradient
